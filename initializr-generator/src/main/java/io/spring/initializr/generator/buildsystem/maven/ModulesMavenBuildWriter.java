...
public class ModulesMavenBuildWriter {
     ...
     */
    public void writeTo(IndentingWriter writer, ModulesMavenBuild build) {
    ...
    private void writeModules(IndentingWriter writer, ModulesMavenBuild build) {
        writer.println();

        if (!build.getSubmodules().isEmpty()) {
            writeElement(writer, "modules", () ->
                    build.getSubmodules().forEach(module -> writeSingleElement(writer, "module", module)));
        }
    }
